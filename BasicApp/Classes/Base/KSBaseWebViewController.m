...
- (void)goRuleVC {
    ...
    webView.titleStr = @"活动规则";
    webView.bannerUrl = [USER_MANAGER isDevStatus] ? SSStr(DevServerURL_H5, SignRuleH5_Url) : SSStr(ServerURL_H5, SignRuleH5_Url);
...
//    NSString *cookieStr = [NSString stringWithFormat:@"ks_http_userName=%@;ks_http_ostype=ios",[USER_MANAGER getUserName]];
...
-(void)dealloc {
    ...
    if(self.lotteryBackRefreshBlock) {
        self.lotteryBackRefreshBlock();
    }
}
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    ...
    {
        //加入QQ群
        //{"k":"VvVm13cayj2ZHb1hwlg90AjGHJjW4Q2_","n":"114580281"}
        NSDictionary *dic = message.body;
        NSString *group = dic[@"n"];
        NSString *key = dic[@"k"];
        [self joinGroup:group key:key];
        
        [[BuryingPointManager shareManager] buryingPointWithEventID:BP_MineQQGroupClick andParameters:@{QQ_GROUP_KEY : key}];
    
    }else if ([message.name isEqualToString:LotteryGetUserMsg]) {
        
        NSDictionary *dic = message.body;
        [self lotteryGetUserMsgRequestWithDic:dic];
        
    }else if ([message.name isEqualToString:LotteryOpenPage]) {
        NSString *type = message.body;
        if ([type isEqualToString:@"signtask"]) {  //做任务赚金币
            __block SignViewController *vc;
            __block BOOL isHave = NO;
            [self.navigationController.viewControllers enumerateObjectsUsingBlock:^(__kindof UIViewController * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                if ([obj isMemberOfClass:[SignViewController class]]) {
                    vc = obj;
                    isHave = YES;
                }
            }];
            ...
            __block MyWalletViewController *vc;
            __block BOOL isHave = NO;
            [self.navigationController.viewControllers enumerateObjectsUsingBlock:^(__kindof UIViewController * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                if ([obj isMemberOfClass:[MyWalletViewController class]]) {
                    vc = obj;
                    isHave = YES;
                }
            }];
...
- (void)lotteryGetUserMsgRequestWithDic:(NSDictionary *)dic {
    BOOL isPost = [dic[@"type"] boolValue];
    NSString *url = dic[@"url"];
    NSString *callBackKey = dic[@"Jscallback"];
    
    NSData *jsonData = [dic[@"Params"] dataUsingEncoding:NSUTF8StringEncoding];
    NSDictionary *par = [NSJSONSerialization JSONObjectWithData:jsonData options:NSJSONReadingMutableContainers error:nil];

    WS()
    [USER_MANAGER requestMsgForH5WithIsPost:isPost url:url params:par success:^(id response) {
