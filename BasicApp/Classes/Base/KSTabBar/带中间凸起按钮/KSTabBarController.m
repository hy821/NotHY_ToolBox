...
- (void)viewDidLoad {
    ...
    [tabbar.plusItem addTarget:self action:@selector(plusAction:) forControlEvents:UIControlEventTouchUpInside];
    [self setValue:tabbar forKeyPath:@"tabBar"];
}

- (void)plusAction:(UIButton *)sender {
    SignViewController *vc = [[SignViewController alloc]init];
    KSBaseNavViewController *nav = [[KSBaseNavViewController alloc]initWithRootViewController:vc];
    [self presentViewController:nav animated:YES completion:nil];
}

-(void)configVC {
    NSArray *classNames = @[@"HomeViewController",@"VideoLibViewController",@"DiscoverViewController",@"MineViewController"];
    NSArray *titles = @[@"首页",@"片库",@"发现",@"我的"];
    NSArray *normalImg = @[@"tab_HomeUn",@"tab_VideoLibraryUn",@"tab_DiscoverUn",@"tab_MineUn"];
    NSArray *selectImg = @[@"tab_Home",@"tab_VideoLibrary",@"tab_Discover",@"tab_Mine"];
